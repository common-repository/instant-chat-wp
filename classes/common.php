...
class InstantChatWPCommon
{
    ...
    {
		global  $instantchatwp;
		$date_format = $instantchatwp->get_option('instantchatwp_date_format');
    ...
    function get_value($option_id) 
	{
        ...
        } elseif (isset($this->instantchatwp_default_options[$option_id]) && $this->instantchatwp_default_options[$option_id] != '' ) {
            return stripslashes($this->instantchatwp_default_options[$option_id]);
        } else {
            return null;
        }
    }
    ...
    function create_plugin_setting($type, $id, $label, $pairs, $help, $inline_help = '', $extra=null) {
		...
		echo wp_kses($cmp, $instantchatwp->allowed_html) ;
        ...
        switch ($type) {
            ...
            case 'textarea':
			  ...
			  case 'textarearich':			  
				...
				$html = $instantchatwp->userpanel->get_me_wphtml_editor($id, $value);				
				//echo wp_kses($html, $instantchatwp->allowed_html) ;	
				echo $html;
        ...
        if (is_array($extra)) {
			...
			echo  wp_kses('<div class="helper-wrap">', $instantchatwp->allowed_html) ;

            foreach ($extra as $a) {
                echo wp_kses($a, $instantchatwp->allowed_html);
            }
           
			echo wp_kses("</div>",  $instantchatwp->allowed_html);
        }       	
		echo wp_kses("</td></tr>",  $instantchatwp->allowed_html);
    }
...
$key = "commmonmethods";
$this->{$key} = new InstantChatWPCommon();
?>